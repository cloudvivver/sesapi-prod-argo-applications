...
# Script para configurar conectores Kafka Connect - Versão corrigida
...
# Configuração do conector Debezium PostgreSQL com ajustes para Azure PostgreSQL
...
echo -e "\nVerificando tópicos no Kafka:"
kubectl exec -n saude-kafka-bigdata kafka-796c4b5567-826z7 -- kafka-topics --bootstrap-server localhost:9092 --list | grep isac || echo "Nenhum tópico isac-cdc encontrado ainda"